...
def reformat_date(
    date_raw: str, input_format: str = "%b %d, %Y", output_format: str = "%Y-%m-%d"
):
...
def format_and_transform(attrs: dict):
    transforms = {
        ...
        "sale_price": lambda raw: float(regex(raw, r"[^\d.]", "split")[1]),
        "original_price": lambda raw: float(regex(raw, r"[^\d.]", "split")[1]),
    }
    ...
    attrs["discount_pct"] = round(
        (attrs["original_price"] - attrs["sale_price"]) / attrs["original_price"] * 100,
        3,
    )
...
def save_to_file(filename="extract", data: list[dict] = None):
    if data is None:
        raise ValueError(
            "The function expects data to be provided as a list of dictionaries"
        )
